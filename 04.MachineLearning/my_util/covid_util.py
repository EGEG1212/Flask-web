...
def get_region_by_date(date):
    ...
    for index, item in enumerate(items):
        ...
        deathCnt = int(item.find('deathCnt').string) if item.find(
            'deathCnt') else 0
        ...
        isolClearCnt = int(item.find('isolClearCnt').string) if item.find(
            'isolClearCnt') else 0
        isolIngCnt = int(item.find('isolIngCnt').string) if item.find(
            'isolIngCnt') else 0
        localOccCnt = int(item.find('localOccCnt').string) if item.find(
            'localOccCnt') else 0
        overFlowCnt = int(item.find('overFlowCnt').string) if item.find(
            'overFlowCnt') else 0
        qurRate = None
...
def make_corona_raw_df(start_date, end_date):
    ...
    cdf_raw = pd.pivot_table(
        df, values='sid', index='확진일', columns='gu', aggfunc='count')
...
def get_new_seoul_data():
    ...
    current_app.logger.debug(
        f'서울시 건수 - {list_total_count}, DB 보관 - {last_sid}')
