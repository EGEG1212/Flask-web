...
def titanic():
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            return redirect(url_for('clsf_bp.titanic'))
...
def pima():
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            flash(
                f'index error : 입력하신 "{index}"인덱스는 존재하지않습니다. 인덱스 범위를 확인하세요.', 'danger')
            return redirect(url_for('clsf_bp.pima'))
        test_scaled = scaler.transform(test_data)
...
def cancer():
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            # 인덱스값에 해당되는 데이터 2차원으로
            test_data = df.iloc[index, :-1].values.reshape(1, -1)
        except:
            current_app.logger.error('index error')
            flash(
                f'index error : 입력하신 "{index}"인덱스는 존재하지않습니다. 인덱스 범위를 확인하세요.', 'danger')
            return redirect(url_for('clsf_bp.cancer'))
        test_scaled = scaler.transform(test_data)  # 그 스케일러로
        label = df.iloc[index, -1]  # 맨끝y값
        ...
        result = {'index': index, 'label': label,
                  'pred_lr': pred_lr[0], 'pred_sv': pred_sv[0], 'pred_rf': pred_rf[0]}
        org = dict(zip(df.columns[:-1], df.iloc[index, :-1]))  # 컬럼명과 값 전달
...
def iris():
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            flash(
                f'index error : 입력하신 "{index}"인덱스는 존재하지않습니다. 인덱스 범위를 확인하세요.', 'danger')
            return redirect(url_for('clsf_bp.iris'))
        test_scaled = scaler.transform(test_data)
        label = df.iloc[index, -1]
...
def wine():
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            flash(
                f'index error : 입력하신 "{index}"인덱스는 존재하지않습니다. 인덱스 범위를 확인하세요.', 'danger')
            return redirect(url_for('clsf_bp.wine'))
        test_scaled = scaler.transform(test_data)
        label = df.iloc[index, -1]
