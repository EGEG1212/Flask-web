...
# 지역별table(달력선택하면 해당일의 '광역시도, 사망자, 확진자, 전일대비, 격리해제, 격리중지역발생, 해외유입')'전지역,합계'있음
...
def daily():
    ...
    return render_template('covid/daily.html', menu=menu, weather=get_weather(),
...
#     return render_template('covid/region.html', menu=menu, weather=get_weather(),
#                            date=date, rows=rows)
...
def region_seq():
    if request.method == 'GET':
        ...
        return render_template('covid/region_seq.html', menu=menu, weather=get_weather(),
...
def age_seq():
    if request.method == 'GET':
        ...
        return render_template('covid/age_seq.html', menu=menu, weather=get_weather(), mtime=mtime, age_list=age_list, age_str=age_str, start_date=start_date, end_date=end_date)
...
#         data = corona_data.get_corona_data(yesrterday_str, yesrterday_str)
#         print(data)
#     return render_template('covid/covid_doughnut.html', menu=menu, weather=get_weather(), data=data[1:])
...
def seoul_seq():
    if request.method == 'GET':
        ...
        return render_template('covid/seoul_seq.html', menu=menu, weather=get_weather(),
...
# 서울시 비교
# 2020.01~12월까지만 있음. 2021년은 없음?ㅋㅋ
...
def seoul_comp():
    if request.method == 'GET':
        ...
        return render_template('covid/seoul_comp.html', menu=menu, weather=get_weather(),
...
def seoul_map(option):
    ...
    return render_template('covid/seoul_map.html', menu=menu, weather=get_weather(),
